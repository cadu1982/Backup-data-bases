...
backup() {
    ...
    mongodump --host ${MONGO_HOST} --port ${MONGO_PORT} --db ${DATABASE_NAMES} --out dumps/<name_db>-${NAME_BACKUP}/
    mv dumps/<name_db>-${NAME_BACKUP}/<name_db>/* dumps/<name_db>-${NAME_BACKUP}
    rmdir dumps/kitnet-${NAME_BACKUP}/kitnet
}

compress(){
    tar -cvzf database.tar.gz  dumps/<name_db>-${NAME_BACKUP}* 
...
upload_s3(){
    ...
    aws s3 cp ${NAME_ARQ}.tar.gz s3://<name_s3> --include ${NAME_ARQ}.tar.gz 
}


download_s3(){
    # recebe backup do S3 AWS - Não esqucer de: export AWS_PROFILE=UserProfile(AWS)
    aws s3 cp s3://<name_s3>/${NAME_REST} ./  
}

list_s3(){
    # Lista arquivos do S3 (AWS)0
    aws s3 ls s3://<name_s3>/ > info.txt
    cut -d' ' -f9 info.txt
    rm -r info.txt
